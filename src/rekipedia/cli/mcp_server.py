...
TOOLS = [
    ...
    {"name": "list_wiki_pages",
     "description": "List all wiki pages generated by rekipedia for this codebase. Returns page names and file paths. Use this to discover what topics are documented before calling get_wiki_page.",
     "inputSchema": {"type": "object", "properties": {
         "repo": {"type": "string", "description": "Absolute path to repo root (default: cwd)"}
     }}},
    {"name": "get_wiki_page",
     "description": "Read the full content of a specific wiki page by name or filename. Returns markdown content with symbol cross-references. Use after list_wiki_pages to get detailed documentation for a module or concept.",
     "inputSchema": {"type": "object", "properties": {
...
def _handle_tool(name: str, args: dict, cache: _StoreCache) -> str:
    try:
        ...
        if name == "get_context":
            ...
            repo_path = Path(args.get("repo") or Path.cwd())
            wiki_dirs = [repo_path / ".rekipedia" / "wiki", repo_path / "docs" / "wiki"]
            pages = []
            for wiki_dir in wiki_dirs:
                if wiki_dir.is_dir():
                    for p in wiki_dir.glob("*.md"):
                        pages.append({"name": p.stem, "path": str(p)})
            pages.sort(key=lambda x: x["name"])
            return json.dumps({"pages": pages})

        elif name == "get_wiki_page":
            page_arg = args.get("page", "").lower()
            repo_path = Path(args.get("repo") or Path.cwd())
            wiki_dirs = [repo_path / ".rekipedia" / "wiki", repo_path / "docs" / "wiki"]
            for wiki_dir in wiki_dirs:
                if wiki_dir.is_dir():
                    for p in sorted(wiki_dir.glob("*.md")):
                        if page_arg in p.stem.lower():
                            return json.dumps({"page": p.stem, "content": p.read_text(encoding="utf-8")})
            # Not found — collect available stems
            available = []
            for wiki_dir in wiki_dirs:
                if wiki_dir.is_dir():
    ...
    config = {
        "mcpServers": {
            "rekipedia": {
                "command": "reki",
                "args": ["mcp"],
                "description": "rekipedia codebase knowledge — ask questions, search symbols, get impact analysis, browse wiki pages",
            }
        }
    }
