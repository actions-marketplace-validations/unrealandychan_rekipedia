...
func makeResult(syms []models.Symbol, rels []models.Relationship, eps []string) models.AnalysisResult {
...
func TestDetectGodClassAboveThreshold(t *testing.T) {
	...
	syms := []models.Symbol{sym("BigClass", "class", "src/big.go")}
	var rels []models.Relationship
	for i := 0; i < 6; i++ {
		rels = append(rels, rel(strings.Repeat("c", i+1), "BigClass", "calls"))
	}
	for i := 0; i < 5; i++ {
		rels = append(rels, rel("BigClass", strings.Repeat("d", i+1), "calls"))
	}
	combined := makeResult(syms, rels, nil)
	...
	if godIssues[0].Metrics.FanIn != 6 {
		t.Errorf("expected fan_in=6, got %d", godIssues[0].Metrics.FanIn)
	}
	if godIssues[0].Metrics.FanOut != 5 {
		t.Errorf("expected fan_out=5, got %d", godIssues[0].Metrics.FanOut)
	}
}

func TestDetectGodClassBelowThreshold(t *testing.T) {
	syms := []models.Symbol{sym("SmallClass", "class", "src/small.go")}
	rels := []models.Relationship{
		rel("A", "SmallClass", "calls"),
		rel("SmallClass", "B", "calls"),
		rel("SmallClass", "C", "calls"),
	}
	combined := makeResult(syms, rels, nil)
	issues := DetectIssues(combined)
...
func TestDetectGodClassCallersCapped(t *testing.T) {
	syms := []models.Symbol{sym("HugeClass", "class", "src/huge.go")}
	var rels []models.Relationship
	for i := 0; i < 30; i++ {
		rels = append(rels, rel(strings.Repeat("x", i+1), "HugeClass", "calls"))
	}
	combined := makeResult(syms, rels, nil)
	issues := DetectIssues(combined)
...
func TestDetectDeadCodeZeroCallers(t *testing.T) {
	syms := []models.Symbol{sym("orphaned_func", "function", "src/app.go")}
	combined := makeResult(syms, nil, nil)
	issues := DetectIssues(combined)
...
func TestDetectDeadCodeWithCallerExcluded(t *testing.T) {
	syms := []models.Symbol{sym("live_func", "function", "src/app.go")}
	rels := []models.Relationship{rel("some_caller", "live_func", "calls")}
	combined := makeResult(syms, rels, nil)
	issues := DetectIssues(combined)
...
func TestDetectDeadCodeSkipsEntryPoints(t *testing.T) {
	syms := []models.Symbol{sym("main_func", "function", "src/main.go")}
	combined := makeResult(syms, nil, []string{"main_func"})
	issues := DetectIssues(combined)
...
func TestDetectDeadCodeSkipsDunder(t *testing.T) {
	syms := []models.Symbol{sym("__init__", "function", "src/app.py")}
	combined := makeResult(syms, nil, nil)
	issues := DetectIssues(combined)
...
func TestDetectDeadCodeSkipsTestHelpers(t *testing.T) {
	syms := []models.Symbol{
		sym("test_something", "function", "src/app.go"),
		sym("TestSomething", "function", "src/app_test.go"),
	}
	combined := makeResult(syms, nil, nil)
	issues := DetectIssues(combined)
...
func TestDetectDeadCodeSkipsTestFileSymbols(t *testing.T) {
	syms := []models.Symbol{sym("helper", "function", "tests/test_util.go")}
	combined := makeResult(syms, nil, nil)
	issues := DetectIssues(combined)
...
func TestIssuesSortedHighBeforeLow(t *testing.T) {
	godSym := sym("GodClass", "class", "src/god.go")
	deadSym := sym("dead_fn", "function", "src/util.go")
	var rels []models.Relationship
	for i := 0; i < 6; i++ {
		rels = append(rels, rel(strings.Repeat("c", i+1), "GodClass", "calls"))
	}
	for i := 0; i < 5; i++ {
		rels = append(rels, rel("GodClass", strings.Repeat("d", i+1), "calls"))
	}
	combined := makeResult([]models.Symbol{godSym, deadSym}, rels, nil)
	issues := DetectIssues(combined)
...
func TestBuildMarkdownHighSection(t *testing.T) {
	issue := RefactorIssue{
		...
		Metrics:  RefactorMetrics{Lines: 300, FanIn: 10, FanOut: 5},
...
func TestBuildMarkdownLowSection(t *testing.T) {
	issue := RefactorIssue{
		...
		Metrics:    RefactorMetrics{FanIn: 0, FanOut: 0},
...
func TestWriteRefactorOutputsCreatesFiles(t *testing.T) {
	dir := t.TempDir()
	combined := makeResult(nil, nil, nil)
...
func TestWriteRefactorOutputsJSONStructure(t *testing.T) {
	dir := t.TempDir()
	syms := []models.Symbol{sym("GodClass", "class", "src/x.go")}
	var rels []models.Relationship
	for i := 0; i < 6; i++ {
		rels = append(rels, rel(strings.Repeat("c", i+1), "GodClass", "calls"))
	}
...
func TestWriteRefactorOutputsSummaryCounts(t *testing.T) {
	dir := t.TempDir()
	syms := []models.Symbol{
		sym("GodClass", "class", "src/x.go"),
		sym("dead_fn", "function", "src/util.go"),
	}
	var rels []models.Relationship
	for i := 0; i < 6; i++ {
		rels = append(rels, rel(strings.Repeat("c", i+1), "GodClass", "calls"))
